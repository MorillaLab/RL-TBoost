...
def classification(method, x_dat, y_dat, model_out=False,
    ...
    if (resultOnly == True) and (test == False):
        ...
        return round(f1_score(y_dat, y_pred, average='micro'),4)
   # round(classification_report(y_dat, y_pred, output_dict=True)['accuracy'],4)
    ...
    if test != False:
        ...
        if (resultOnly == True):
            ...
            return round(f1_score(y_test,y_pred_test, average='micro'),4)
        else:
            print("Results for {}:".format(method.__name__))
...
def best_threshold(model,x,y):
    ...
    for t in thresh:
        ...
        accuracy.append(round(classification_report(y, y_pred_thresh, output_dict=True)['accuracy'],4))
        recall.append(round(classification_report(y, y_pred_thresh, output_dict=True)['1']['recall'],4))
